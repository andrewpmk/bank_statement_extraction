#!/usr/bin/env python3
"""Convert Simplii Financial credit card statement PDFs into CSVs using OpenRouter (model default: openai/gpt-4o-mini).

Output files are named: simplii_YYYYMMDD_YYYYMMDD.csv (start/end dates from PDF or filename).

Columns (headers): Date,Posted Date,Description,Category,Amount
Date format in CSV: MM/DD/YYYY
Amount: CAD amount (positive for purchases, negative for payments/credits)
"""
...
def _billing_cycle_from_end_simplii(end_year: int, end_month: int) -> tuple[datetime, datetime]:
    """Simplii statements run from the 13th of the previous month to the 12th of end_month."""
    end = datetime(end_year, end_month, 12)
    prev_month = end_month - 1 or 12
    prev_year = end_year if end_month != 1 else end_year - 1
    start = datetime(prev_year, prev_month, 13)
    return start, end


def find_statement_period_in_text(text: str, pdf_path: Path | None = None) -> tuple[datetime, datetime] | None:
    # 1) Try to parse from filename: simplii_YYYY_MM.pdf
    if pdf_path is not None:
        m = re.search(r"simplii[_-](\d{4})[_-](\d{2})\b", pdf_path.name, re.I)
        if m:
            try:
                y = int(m.group(1))
                mo = int(m.group(2))
                return _billing_cycle_from_end_simplii(y, mo)
            except Exception:
                pass

    # 2) Try to parse "Month DD to Month DD, YYYY" from PDF text
    #    e.g. "June 13 to July 12, 2024" or "July 13to August 12, 2024"
    pat = re.compile(
    ...
    if m:
        try:
            ...
            m2 = _MON.get(m.group(3).lower()[:3])
            y = int(m.group(5))
            if m1 and m2:
                start = datetime(y, m1, int(m.group(2)))
                # end year may differ if spanning Dec->Jan
                end_y = y if m2 >= m1 else y + 1
                end = datetime(end_y, m2, int(m.group(4)))
                return start, end
        except Exception:
            pass

    # 3) Fallback: look for "MONTH statement period" heading to infer end month
    m2 = re.search(r"([A-Za-z]{3,9})\s+statement\s+period", text, re.I)
    if m2:
        mo_name = m2.group(1).lower()[:3]
        mo = _MON.get(mo_name)
        # guess year from PDF path if available
        year = None
        if pdf_path is not None:
        ...
        if mo and year:
            try:
                return _billing_cycle_from_end_simplii(year, mo)
            except Exception:
                pass
...
def build_prompt(start: datetime, end: datetime) -> str:
    rules = (
        "You are given images of a Simplii Financial Cash Back Visa credit card statement "
        "together with the machine-extracted text from the same PDF. "
        "Extract all posted transactions from the 'Your new charges and credits' section as a JSON array. "
        "Each object must have EXACT keys: Date, PostedDate, Description, Category, Amount. "
        ...
        "Amount should be the CAD dollar amount as a decimal (e.g. 23.72). "
...
def output_csv_path(pdf_path: Path, start: datetime, end: datetime, output_dir: Path | None) -> Path:
    filename = f"simplii_{start.strftime('%Y%m%d')}_{end.strftime('%Y%m%d')}.csv"
    if output_dir:
...
def parse_fx_from_text(text: str) -> list[dict[str, str]]:
    ...
    Simplii prints a second line immediately after a foreign currency transaction:
        13.56 USD @ 1.418141593**
    This function finds those lines and pairs them with the CAD amount on the
    preceding transaction line, returning a list of dicts with keys:
    cad_amount, orig_amount, currency, exchange_rate.
    """
    results: list[dict[str, str]] = []
    lines = text.splitlines()
    # Pattern: "<orig> <CUR> @ <rate>" optionally followed by "**"
    ...
    for i, line in enumerate(lines):
        ...
        if m and i > 0:
            ...
            currency = m.group(2)
            rate = m.group(3)
            # The preceding line ends with the posted CAD amount
...
def apply_fx_to_rows(out_rows: list[list[str]], fx_list: list[dict[str, str]]) -> list[list[str]]:
    """Fill in OriginalAmount/Currency/ExchangeRate for any rows where Currency is
    blank but whose Amount CAD matches a foreign currency entry parsed from the text.

    Row layout: [date, posted, desc, category, orig_amount, currency, exchange_rate, amt_cad]
    """
    if not fx_list:
        return out_rows
    # Build lookup by CAD amount string; keep a consumed set to avoid double-applying
    # when two transactions coincidentally share the same CAD amount
    fx_by_cad: dict[str, list[dict[str, str]]] = {}
    for fx in fx_list:
        fx_by_cad.setdefault(fx['cad_amount'], []).append(fx)
    used: dict[str, int] = {}  # cad_amount -> next index into fx_by_cad[cad_amount]
    for row in out_rows:
        # Only fill in rows that are missing currency data
        if row[5]:  # currency already set
            continue
        amt_cad = row[7]  # Amount CAD column
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Convert Simplii Financial credit card statement PDFs with OpenRouter-assisted parsing (default: openai/gpt-4o-mini)."
    )
