#!/usr/bin/env python3
"""Convert Tangerine chequing/savings statement PDFs into accountactivity CSVs using OpenRouter.

This is adapted from the TD converter. It decides account type from the FILENAME
only (looks for 'chequing' vs 'saving'/'estatement'). Outputs files named:
  accountactivity_tangerine_chequing_YYYY_MM.csv
  accountactivity_tangerine_savings_YYYY_MM.csv
in the same row format as the TD converter: Date, Description, Withdrawals, Deposits, Balance
"""
...
def parse_statement_period(pdf_path: Path) -> StatementPeriod:
    # Look for patterns like Apr25, APR25, Apr2025, etc.
    m = re.search(r"([A-Za-z]{3})(\d{2,4})", pdf_path.name)
    if not m:
        raise ValueError(f"Unable to parse statement period from filename: {pdf_path.name}")
    mon = m.group(1).upper()
    yrtok = m.group(2)
    if mon not in MONTH_TO_NUM:
        raise ValueError(f"Unrecognized month in filename: {pdf_path.name}")
    month_num = MONTH_TO_NUM[mon]
    year = int(yrtok)
    if year < 100:
        year += 2000
    return StatementPeriod(end_month=month_num, end_year=year)


def detect_account_type(pdf_path: Path) -> str:
    name = pdf_path.name.lower()
    if "chequ" in name or "checking" in name:
        return "chequing"
    # treat generic eStatement or 'savings' as savings
    if "saving" in name or "estatement" in name or "e-statement" in name:
        return "savings"
    # default to savings when uncertain
    return "savings"
...
def build_prompt(period: StatementPeriod) -> str:
    rules = (
        "You extract Tangerine statement transactions from page images. "
...
def output_csv_path(pdf_path: Path, period: StatementPeriod, acct_type: str, output_dir: Path | None) -> Path:
    filename = f"accountactivity_tangerine_{acct_type}_{period.end_year:04d}_{period.end_month:02d}.csv"
    if output_dir:
...
def convert_file(pdf_path: Path, output_dir: Path | None, model: str, api_key: str, timeout_s: int) -> Path:
    ...
    acct_type = detect_account_type(pdf_path)
    page_data_urls = render_pdf_pages_as_data_urls(pdf_path)
    prompt = build_prompt(period)
    model_text = call_openrouter(model, api_key, prompt, page_data_urls, args.timeout if 'args' in globals() else timeout_s)
    ai_rows = extract_json_array(model_text)
    out_rows = clean_ai_rows(ai_rows, period)

    out_path = output_csv_path(pdf_path, period, acct_type, output_dir)
