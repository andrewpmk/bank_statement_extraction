#!/usr/bin/env python3
"""Convert Amex credit card statement PDFs into CSVs using OpenRouter (model default: openai/gpt-4o-mini).

Output files are named: amex_YYYYMMDD_YYYYMMDD.csv (start/end dates from PDF).
...
def _billing_cycle_from_end(end_year: int, end_month: int, end_day: int) -> tuple[datetime, datetime]:
    """Given an end date, return (start, end) using 21st-prev-month → 20th-this-month cycle."""
    end = datetime(end_year, end_month, end_day)
    prev_month = end_month - 1 or 12
    prev_year = end_year if end_month != 1 else end_year - 1
    start = datetime(prev_year, prev_month, 21)
    return start, end


def find_statement_period_in_text(text: str, pdf_path: Path | None = None) -> tuple[datetime, datetime] | None:
    # ── 1. Explicit "Statement Period ... to ..." text ─────────────────────────
    for pat in [
    ...
    # ── 2. Filename-derived date (most reliable for 2021+ PDFs) ────────────────
    # 2a. YYYY-MM-DD.pdf  →  the date IS the statement end date (always 20th for Amex)
    if pdf_path is not None:
        m = re.fullmatch(r"(\d{4})-(\d{2})-(\d{2})\.pdf", pdf_path.name, re.I)
        if m:
            try:
                return _billing_cycle_from_end(int(m.group(1)), int(m.group(2)), int(m.group(3)))
            except Exception:
                pass

    # 2b. "Statement_MMM YYYY.pdf"  →  end = 20th of that month
    if pdf_path is not None:
        m2 = re.search(r"Statement[_\s-]*([A-Za-z]{3,9})[_\s-]*(\d{4})", pdf_path.name, re.I)
        if m2:
            _MON = {"jan":1,"feb":2,"mar":3,"apr":4,"may":5,"jun":6,
                    "jul":7,"aug":8,"sep":9,"oct":10,"nov":11,"dec":12}
            mon_num = _MON.get(m2.group(1).lower()[:3])
            if mon_num:
                try:
                    return _billing_cycle_from_end(int(m2.group(2)), mon_num, 20)
                except Exception:
                    pass

    # ── 3. Last resort: look only at first 800 chars (header area) for named dates ─
    header = text[:800]
...
def build_prompt(start: datetime, end: datetime) -> str:
    rules = (
        "You are given images of an American Express (Amex) credit card statement. Extract all posted transactions as a JSON array."
...
def normalize_rate(value: str) -> str:
    """Normalize exchange rate without rounding: remove commas and surrounding whitespace, preserve precision."""
    if value is None:
        return ""
    v = str(value).strip()
...
def normalize_description(value: str) -> str:
    """Collapse repeated whitespace in descriptions to single spaces and trim."""
    if value is None:
        return ""
...
def output_csv_path(pdf_path: Path, start: datetime, end: datetime, output_dir: Path | None) -> Path:
    filename = f"amex_{start.strftime('%Y%m%d')}_{end.strftime('%Y%m%d')}.csv"
    if output_dir:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Convert Amex credit card statement PDFs with OpenRouter-assisted parsing (default: openai/gpt-4o-mini).")
